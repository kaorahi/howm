...
(defun howm-one-file-one-day-p ()
  (let* ((now (decode-time))
         ...
         (m (nth 4 now))
         (y (nth 5 now))
         (beginning-of-day (howm-encode-time 0 0 0 d m y))
         (end-of-day (howm-encode-time 59 59 23 d m y)))
...
(defun howm-recent-menu (num &optional evaluator)
  ...
  (let* ((randomp (eq evaluator t))
    ...
    (let* ((r (howm-menu-recent-regexp))
      ...
      (if randomp
          (cl-mapcan (lambda (f)
                       (let ((is (funcall searcher f)))
                         (and is (list (nth (random (length is))
                                            is)))))
                     files)
        (howm-first-n (cl-mapcan searcher files) num)))))
