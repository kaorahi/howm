...
(defcustom howm-view-close-frame/tab-on-exit nil
  "Close the frame/tab when exiting from howm summary/contents buffers."
  :type '(radio (const :tag "Default" nil)
                (const :tag "Close" t)
...
(defun riffle-kill-buffer (&rest args)
  ...
  (let* ((howm-window-p
         ...
         (delete-frame/tab-p
          (cond ((eq howm-view-close-frame/tab-on-exit t) t)
                ((eq howm-view-close-frame/tab-on-exit 'howm-only)
